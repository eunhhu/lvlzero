    ...
    tick(levels, enemies, delta) {
        if (this.status === "waiting") {
            ...
            if (this.waitingTimer <= 0) {
                ...
                const enems = this.parseEnemyRegex(lvl.enemyRegexes[this.wave]).map((enemyType) => {
    ...
    command(units, levels, command) {
        ...
        switch (commander) {
            ...
            case 'set':
                ...
                switch (params[1]) {
                    case 'wave':
                        if (+params[2] < 0 || isNaN(+params[2]))
                            return;
                        this.wave = +params[2];
                        break;
                    case 'level':
                        if (+params[2] < 1 || isNaN(+params[2]))
                            return;
                        this.level = +params[2];
                        break;
                    case 'health':
